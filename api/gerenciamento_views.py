...
def criar_pessoa(request):
    try:
        ...
        tipo = dados.get('tipo')
        if not nome or not documento or not tipo:
            return JsonResponse({'error': 'Preencha nome, documento e tipo.'}, status=400)
        existente = Pessoas.objects.filter(documento=documento, tipo=tipo).first()
        if existente:
            return JsonResponse({'error': 'Documento já cadastrado para este tipo.'}, status=400)
        pessoa = Pessoas.objects.create(nome=nome, documento=documento, tipo=tipo, ativo=True)
        return JsonResponse({'success': True, 'id': pessoa.id})
...
def criar_classificacao(request):
    try:
        ...
        if not descricao or not tipo:
            return JsonResponse({'error': 'Preencha descrição e tipo.'}, status=400)
        classificacao = Classificacao.objects.create(descricao=descricao, tipo=tipo, ativo=True)
        return JsonResponse({'success': True, 'id': classificacao.id})
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)
...
@csrf_exempt
@require_http_methods(["PATCH"])
def alterar_status_movimento(request, movimento_id):
    try:
        movimento = get_object_or_404(MovimentoContas, id=movimento_id)
        dados = json.loads(request.body)
        movimento.ativo = dados.get('ativo', True)
        movimento.save()
        return JsonResponse({'success': True})
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)
...
@csrf_exempt
@require_http_methods(["PATCH"])
def alterar_status_parcela(request, parcela_id):
    try:
        parcela = get_object_or_404(ParcelasContas, id=parcela_id)
        dados = json.loads(request.body)
        parcela.ativo = dados.get('ativo', True)
        parcela.save()
        return JsonResponse({'success': True})
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)
