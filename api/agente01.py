...
def ensure_required_shape(data, raw_json_text=None):
    ...
    nf_raw = data.get('nota_fiscal', {}) or {}
    nf = nf_raw[0] if isinstance(nf_raw, list) and nf_raw else (nf_raw if isinstance(nf_raw, dict) else {})
...
def extrair_dados_nfe(file_content, file_name):
    ...
    try:
        ...
        try:
            ...
            response_text = None
        # fallback: tentar montar texto a partir das partes
        ...
        if not response_text:
            return {
                ...
                "details": "Não foi possível obter texto da resposta"
            }
        ...
        # Ajustar formato quando vier lista
        if isinstance(resultado_json, list):
            ...
            if candidato is None:
                return {
                    ...
                    "details": "Esperado objeto JSON, recebido lista sem objetos"
                }
            resultado_json = candidato
